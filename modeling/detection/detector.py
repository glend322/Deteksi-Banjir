...
Full pipeline: CCTV → CV (flood + cause) → Verify → Classify → Output
...
BASELINE_FILE = ROOT_DIR / "data" / "camera_baselines.json"
...
class BaselineTracker:
    """Track baseline water levels per camera to detect real flood vs normal conditions."""
    ...
    def _load(self):
        if self.baseline_file.exists():
            ...
            logger.info(f"Loaded baselines for {len(self.baselines)} cameras")
    ...
    def get_water_ratio(self, frame_bytes: bytes) -> float:
        """Calculate water area ratio from frame."""
        ...
        return float(np.sum(water_mask > 0)) / water_mask.size

    def update_baseline(self, camera_key: str, water_ratio: float):
        """Update baseline using exponential moving average."""
        if camera_key not in self.baselines:
            self.baselines[camera_key] = {
                "mean_water_ratio": water_ratio,
                "count": 1,
                "last_update": time.time(),
            }
        else:
            b = self.baselines[camera_key]
            alpha = 0.1
            b["mean_water_ratio"] = alpha * water_ratio + (1 - alpha) * b["mean_water_ratio"]
            b["count"] += 1
            b["last_update"] = time.time()

    def is_real_flood(self, camera_key: str, current_ratio: float, threshold: float = 0.3) -> bool:
        """Check if current water level is significantly above baseline."""
        if camera_key not in self.baselines:
            return current_ratio > 0.3

        baseline = self.baselines[camera_key]["mean_water_ratio"]
        increase = current_ratio - baseline

        if baseline > 0.4:
            return increase > 0.2

        return increase > threshold and current_ratio > 0.2
...
class FloodDetectionPipeline:
    """
    CCTV → CV Detection (flood + cause) → Verification → Classification → Output
    """
    ...
    def _init(self):
        ...
        if ckpt_path.exists():
            ...
            self._cv_model = FloodClassifier(
                num_classes=2, num_causes=2, pretrained=True, backbone="mobilenet_v3_small"
            )
    ...
    def _run_cv_inference(self, frame_bytes: bytes) -> dict:
        ...
        flood_prob = float(flood_probs[1]) if len(flood_probs) > 1 else float(flood_probs[pred_class])

        flood_detected = pred_class == 1 or flood_prob > 0.5

        cause_dict = {name: float(prob) for name, prob in zip(CAUSE_NAMES, cause_probs)}
        river_detected = cause_dict.get("river", 0.0) > 0.5
        trash_detected = cause_dict.get("trash", 0.0) > 0.5
    ...
    async def detect_single_camera(self, camera) -> Optional[FloodDetection]:
        try:
            ...
            water_ratio = self._baseline.get_water_ratio(frame_bytes)
            ...
            if is_flood:
                ...
                cls_result.cause_text = ""
                cls_result.river_detected = False
                cls_result.trash_detected = False
            ...
            return FloodDetection(
                ...
                cause_text=cls_result.cause_text,
